...
num_k = 5
reg_coef = 0.1
tb_dir = '/data/tensorboard_log/leewk92/gan/dcgan/animation/'
checkpoint_dir = './checkpoints/dcgan_anim/'
is_training = True

#mnist = input_data.read_data_sets('../../mnist/', one_hot = True)
#yaleface = YaleFace(height=32, width=32)
anim = Animation(height=height, width=width)
    ...
    with tf.variable_scope('generator'):
        ...
        with tf.variable_scope('transposed_conv2d'):
            with slim.arg_scope([layers.conv2d_transpose],
                ...
                conv4 = tf.nn.relu(layers.batch_norm(conv4, is_training=is_training))
                generated = layers.conv2d_transpose(conv4, num_outputs=3,
                                                    kernel_size=[5,5],
                                                    stride=[2,2],
                                                    scope='generated',
...
def get_next_batch(batch_size):
    samples = anim.next_batch(batch_size)
    return samples
